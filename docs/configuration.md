...
- Errors related to scaling are retried via SQS. Configure `redrive_build_queue` with a `maxReceiveCount` to prevent unbounded retries on persistent errors. Without a dead letter queue, failed messages retry until `job_queue_retention_in_seconds` expires, which can cause sustained Lambda invocations and log volume. You can also configure `job_queue_retention_in_seconds` to tune retention. We have no mechanism to avoid events never being processed, which means potentially no runner gets created and the job in GitHub times out in 6 hours.
